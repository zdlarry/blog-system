...
const addSelectDate = () => {
	getData('../visualization/data_generate/dates.json').then(dates => {
		$('#selectpicker').html('')
...
const initial = date => {
	$.when(
		getData('../visualization/data_generate/' + date + '_type_lng_lat.json'),
		getData('../visualization/data_generate/' + date + '_time_type.json'),
		getData('../visualization/data_generate/' + date + '_time_type_lng_lat.json'),
		getData('../visualization/data_generate/' + date + '_type_time_lng_lat.json'),
		getData('../visualization/map_setup/map_style.json'),
		getData('../visualization/words_freq/' + date + '.json'),
		getData('../visualization/data_generate/' + date + '_area_type.json'),
		getData('../visualization/data_generate/dates.json'),
		getData('../visualization/data_generate/' + date + '_area_time.json')
...
const generate_word_series = (datas, maskImage) => {
	return {
		type: 'wordCloud',
	    sizeRange: [6, 47],
	    rotationRange: [-90, 90],
...
const switch_chart = (datas, words_cloud_chart, type) => {
	let txt, series, maskImage = new Image()
	if (type == 'pie') {
		txt = 'Pie Chart'
		series = generate_pie_series(datas)
	} else {
		txt = 'Words Cloud Chart'
		series = generate_word_series(datas, maskImage)
	}
	option = {
		title: {
   			text: txt,
	...
	if (type == 'pie') {
	    ...
	    maskImage.src = '../visualization/imgs/_logo.png'
	}
